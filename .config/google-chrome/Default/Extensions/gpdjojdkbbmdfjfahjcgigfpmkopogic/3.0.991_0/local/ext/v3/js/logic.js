// test new iframed pin create form

// our logic version
const VERSION = 2018083001;

// set CAN_HAZ_PFOB to false to kill all personalization from offsite browsing
const CAN_HAZ_PFOB = true;

// set me to the path of your choice to try the new iframed pin create form
// 0 = not testing
const IFRAME_CREATE_PATH = 0;
// const IFRAME_CREATE_PATH = 'https://www.pinterest.com/browserbutton/create_pin/';
...
((w, d, a) => {
  var $ = w[a.k] = {
    ...
    v: {
      ...
      iframeCreatePath: IFRAME_CREATE_PATH,
      nopin: false,
      observer: {},
      meta: {}
    },
    ...
    f: {
      ...
      openOverlay: o => {
        ...
        if (o.id && !$.s[o.id]) {
          ...
          if (o.id === 'create' && $.v.iframeCreatePath) {
	          path = $.v.iframeCreatePath;
          } else {
	          path = $.b.extension.getURL(path);
          }
          ...
          $.s[o.id].onload = () => {
            ...
            if (o.data) {
              if (o.id === 'create' && $.v.iframeCreatePath) {
                ...
                $.f.debug('Posting message to Pinterest-hosted iframe.');
                $.f.debug($.s[o.id].src);
                $.f.debug(postMessageObj);
                $.s[o.id].contentWindow.postMessage(postMessageObj, $.s[o.id].src);
              } else {
                // native overlay
                o.data.meta = $.v.meta;
	              $.f.sendMessage({
