// handle 401s and 404s for board-not-found errors
          ...
          return new Promise((win, fail) => {
            ...
            req.onload = () => {
              if (req.status === 200) {
                ...
                // wipe local copy of boards if we get an unauthorized error
                // note: we can't exact-match for a 401 because we get a 404 when trying to save to a
                // secret board that we don't own
                if (req.status > 399) {
                  $.f.log({
                    event: 'auth_fail'
                  });
              ...
              $.b.cookies.get({'name': '_auth', url: `https://${$.v.wwwDomain}/`}, (r) => {
                if (r && r.value === "1") {
                  ...
                  $.b.cookies.get({'name': 'csrftoken', url: `https://${$.v.csrfDomain}/`}, (r) => {
                    if (r && r.value) {
                      $.v.csrfToken = r.value;
                      $.f.debug('CSRF token found: ' + $.v.csrfToken);
                    } else {
