...
function lowAccount(emailConfirmed) {
    if (emailConfirmed === true) {
        ...
        browser.tabs.create({ url: BASE_URL + "/account#/upgrade?notice=confirm_email&v=" + utils["p" /* versionString */] });
    }
}
...
var token_manager_TokenManager = /** @class */ (function () {
    ...
    TokenManager.prototype.newToken = function (proxy) {
        ...
        if (this.requestInProgress === false || this.registerPromise === undefined) {
            ...
            this.registerPromise = register(true, 0).then(function (response) {
...
function toggleIcon(isToggled) {
    if (isToggled) {
        browser.browserAction.setIcon({
            path: {
                19: __webpack_require__(63),
                38: __webpack_require__(62)
            }
        });
    }
    else {
        browser.browserAction.setIcon({
            path: {
                19: __webpack_require__(61),
                38: __webpack_require__(60)
            }
        });
    }
}
...
function shouldReset(servers) {
    return servers.map(function (server) { return proxy_lastServers.indexOf(server) >= 0; }).indexOf(false) >= 0;
}
...
function core_portCallback(portName, message) {
    switch (portName) {
        ...
        case ports["a" /* PortName */].UPGRADE: {
            upgradeAccount(regResponse.emailConfirmed);
            break;
        }
...
function toggleCountryParam(country) {
    register(false, country).then(function () {
        updateProxyAll();
        getLocation();
    });
